...
- **用户主导执行**：后端不存储私钥/API 凭证，所有下单需用户显式确认。
- **确定性策略流程**：策略变更需在 spec/plan 中锁定目标、预算与回滚方案。
- **类型化市场数据合约**：订单簿、成交、余额等关键结构须定义 TypeScript 类型与运行时校验。
- **测试优先自动化**：为每个风险场景编写先失败的测试用例后再实现。
- **可观测性与告警**：为 WS 连接、策略心跳、风控事件提供结构化日志和告警。
